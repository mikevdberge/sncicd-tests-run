...
class App {
    ...
    constructor(props) {
        ...
        axios_1.default.defaults.headers.common['User-Agent'] = 'sncicd_extint_github';
        ...
        if (this.apikey) {
            ...
            //     }
            // }
            axios_1.default.defaults.headers.common['x-sn-apikey'] = this.apikey;
        }
        if (this.hmactoken) {
            // this.config = {
            //     headers: {
            ...
            //     }
            // }
            axios_1.default.defaults.headers.common['x-sn-hmac-signature-256'] = this.hmactoken;
        }
        ...
        if (!this.apikey && !this.hmactoken) {
            ...
            axios_1.default.defaults.headers.common['Authorization'] = auth;
        }
    }
    buildParams(options) {
    ...
    async printStatus(result) {
        ...
        if (+result.status < App_types_1.ResponseStatus.Successful) {
            const response = await axios_1.default.get(result.links.progress.url, {
                data: this.body
            });
            // Throttling
